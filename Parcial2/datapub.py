...
def mining_pubs(keyword,field):
   ...
   #POSIBLE MODIFICACION 
    ...
    records=[]
    for item in idlist:
        with en.efetch(db="nucleotide", rettype="gb", retmode="text", id=item)as handle:
            records.append(md.read(handle))
    ...
    if {field}=='AU':
        x=pd.DataFrame(list(zip(pmid_dict,contador)),columns=['PMID','num_auth'])
    elif {field}=='AD':
        x=pd.DataFrame(list(zip(ad_dict,contador)),columns=['country','num_auth'])
    elif {field}=='DP': 
        x=pd.DataFrame(list(zip(pmid_dict,dp_dict)),columns=['PMID','DP_year'])
